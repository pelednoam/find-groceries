    ...
    for (const [store, items] of Object.entries(data().items)) {
        for (const [name, cell] of Object.entries(items)) {
            if (name === term || name.includes(term) || term.includes(name)) {
                ...
                const rank = (n) => (n === term ? 2 : n.includes(term) ? 1 : 0);
                const better = !prev
 ...
 */
function rankStores(items, pref) {
...
function renderBasket() {
    ...
    for (const item of basket) {
        ...
        const label = resolved.kind === "none" ? "no evidence either way"
            : resolved.kind === "category" ? "matched to " + titleCase(Object.values(resolved.perStore)[0]?.label ?? "")
...
function runList() {
    ...
    if (best) {
        ...
        note.append(el("p", { text: `Scores ${fmt(best.avg)} across your basket with evidence for `
...
function compareRows() {
    ...
    const value = (r) => k === "price" ? (r.price === undefined ? undefined : -r.price)
        : k === "quality" ? r.quality
...
function renderCross() {
    ...
    for (const row of rows) {
        const tr = el("tr", {}, [
            ...
            el("td", { class: "num muted", text: (need("#cross-pop").value === "norm_long"
...
function placeScore(place) {
    ...
    if (cat) {
        ...
        if (branchCell)
            return { cell: branchCell, label: cat };
        const chainCell = chainCats?.[cat];
        if (chainCell)
            return { cell: chainCell, label: `${cat}, chain-wide` };
    }
...
function openDetail(store, branch) {
    closeDrawer();
    ...
    if (rating && totals) {
        ...
        box.append(el("div", { class: "gap-note", text: Math.abs(gap) < 0.15
    ...
    panel.append(el("p", { class: "fineprint", text: "Figures are weighted opinion, not measurements. Every quote links to the "
            + "comment it came from." }));
    document.body.append(scrim, panel);
    drawerOpen = [scrim, panel];
    panel.focus();
}
function sourceBar(label, value, kind) {
...
function renderStoreCards() {
    ...
    clear(wrap);
    const stores = Object.keys(data().stores).sort((a, b) => (data().totals[b]?.w ?? 0) - (data().totals[a]?.w ?? 0));
    for (const store of stores) {
        ...
        if (rating)
            bars.append(sourceBar("Google", rating.norm, "google"));
        card.append(bars);
        ...
        if (best)
            card.append(el("p", { class: "quote", text: "“" + best.t + "”" }));
...
/* ── boot ───────────────────────────────────────────────────────────── */
const VIEWS = ["list", "compare", "cross", "map", "items", "method"];
...
async function boot() {
    try {
        ...
        if (!isPayload(parsed))
            throw new Error("verdicts.json is not the expected shape");
        DATA = parsed;
    }
    catch (err) {
    ...
    initScanner();
    const go = (v) => { switchView(v); renderView(v); };
    ...
    for (const [id, label] of sorts) {
        ...
        b.addEventListener("click", () => {
            sortMode = id;
            for (const other of Array.from(bar.children))
...
function matchTerm(text) {
    ...
    for (const items of Object.values(data().items)) {
        for (const name of Object.keys(items)) {
            if (text.includes(name) && (!best || name.length > best.length))
                best = name;
        }
    }
    return best;
}
function parseLines(receiptText) {
    const v = vocab();
    ...
    for (const raw of receiptText.split(/\r?\n/)) {
        ...
        if (!text || (price === null && !matchTerm(text)))
            continue;
...
function detectStore(receiptText) {
...
function detectDate(receiptText) {
...
function loadHistory() {
    try {
        ...
        if (!Array.isArray(parsed))
            return [];
        return parsed.filter((r) => typeof r === "object" && r !== null && Array.isArray(r.lines));
    }
    catch {
 ...
 */
function learn(history, now = Date.now() / 1000) {
    const by = new Map();
 ...
 */
function basketFromHistory(learned, limit = 12) {
    return learned
...
async function readImage(file, onProgress) {
    ...
    await loadOcr();
    if (typeof Tesseract === "undefined")
        throw new Error("the reader did not load");
...
function scanProgress(pct, label) {
    const box = need("#scan-progress");
    box.hidden = false;
    need("#scan-bar").style.width = Math.max(0, Math.min(100, pct)) + "%";
...
function openScanner() {
    const modal = need("#scan-modal");
    modal.hidden = false;
    document.body.classList.add("modal-open");
    need("#scan-close").focus();
}
function closeScanner() {
    need("#scan-modal").hidden = true;
    document.body.classList.remove("modal-open");
    need("#scan-open").focus();
}
...
function stageFile(file) {
    ...
    need("#dropzone").classList.add("loaded");
    scanHint("");
}
function renderReview() {
    const list = need("#scan-lines");
    clear(list);
    need("#scan-review").hidden = pending.length === 0;
    need("#scan-save").hidden = pending.length === 0;
    ...
    for (const [i, line] of pending.entries()) {
        ...
        const box = el("input", { type: "checkbox", class: "tick",
            "aria-label": `Remember ${line.match ?? line.text}` });
        ...
        li.append(box, body);
        if (line.qty > 1)
            li.append(el("span", { class: "rv-qty", text: "×" + line.qty }));
...
async function runScan() {
    ...
    try {
        ...
        need("#scan-progress").hidden = true;
...
function resetScanner() {
    ...
    pendingStore = null;
    pendingAt = null;
    need("#scan-text").value = "";
    need("#scan-review").hidden = true;
    need("#scan-save").hidden = true;
    need("#scan-progress").hidden = true;
    need("#dropzone").classList.remove("loaded");
...
function renderLearned(learned, receipts) {
    const panel = need("#learned-panel");
...
function initScanner() {
    ...
    const zone = need("#dropzone");
    const file = need("#scan-file");
    ...
    need("#history-clear").addEventListener("click", () => {
        ...
        need("#learned-panel").hidden = true;
